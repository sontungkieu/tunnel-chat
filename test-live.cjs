...
const req = https.request({ host: HOST, servername: HOST, port: 443, path: '/', method: 'GET', headers: { host: HOST },
  ...
  res.on('end', function () { console.log('tunnel ->', res.statusCode, '|', d.length, 'B | shim:', d.indexOf('__dsh_bridge/ws-shim.js') !== -1); });
});
