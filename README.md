...
FILE_TRANSFER_MAX_BYTES=134217728
FILE_TRANSFER_STAGING_MAX_BYTES=536870912
TRANSFER_PROTOCOL=auto
BINARY_UPLOAD_CHUNK_BYTES=8192
```
...
- `FILE_TRANSFER_MAX_BYTES`: maximum upload size per file, default 128 MiB for
  binary-v2. It can be raised to 10 GiB together with the staging quota.
  Downloads are streamed without this upload limit.
- `FILE_TRANSFER_STAGING_MAX_BYTES`: total temporary disk budget for incomplete
  binary uploads; default 512 MiB.
...
For the included 128 MiB profile use `TRANSFER_PROTOCOL=auto`,
`BINARY_UPLOAD_CHUNK_BYTES=8192`, `UPLOAD_CONCURRENCY=4`,
`UPLOAD_CONCURRENCY_MAX=8`, `FILE_TRANSFER_MAX_BYTES=134217728`, and
`FILE_TRANSFER_STAGING_MAX_BYTES=536870912`. Raise both file and staging limits
together if larger files are required. Traffic remains on the existing
