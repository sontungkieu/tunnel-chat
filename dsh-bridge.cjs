...
function diagPage() {
  const lines = [
    ...
    '  try {',
    '    var sizes = [4096, 8192, 16384, 32768, 65536, 131072, 262144, 1048576, 2097152];',
    '    var maxOk = 0, failAt = 0;',
    '    for (var si = 0; si < sizes.length; si++) {',
    '      try {',
    '        var bb = new Uint8Array(sizes[si]);',
    '        var rb = await fetch("/__dsh_bridge/diag/echo", { method:"POST", body: bb, cache:"no-store" });',
    '        var jb = await rb.json();',
    '        if (jb.bytes === sizes[si]) { maxOk = sizes[si]; } else { failAt = sizes[si]; break; }',
    '      } catch (e) { failAt = sizes[si]; break; }',
    '    }',
    '    row("Tran body POST", maxOk ? ((maxOk/1024) + " KB gui duoc") : "khong gui duoc",',
    '      failAt ? ("vuot " + (failAt/1024) + " KB la bi chan") : "khong cham tran trong 2 MB",',
    '      maxOk >= 1048576);',
    '  } catch (e) { row("Tran body POST", String(e), "loi do", false); }',
