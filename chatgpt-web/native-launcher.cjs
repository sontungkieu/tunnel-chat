...
async function main() {
  ...
  const response = await fetch(config.origin + '/chat/api/status', { signal: AbortSignal.timeout(5000) });
  if ((await response.json()).ready) { console.log('Native browser helper already connected'); return; }
  ...
  const child = spawn(process.execPath, ['--experimental-websocket',path.join(__dirname,'native-driver.cjs'),configPath],
