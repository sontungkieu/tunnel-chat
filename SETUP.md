...
- `/files`: kho truyền file hai chiều riêng, mặc định chỉ mở
  ...
  `FILE_TRANSFER_STAGING_MAX_BYTES` trong `.env.local`; cấu hình mặc định cho
  phép tối đa 128 MiB mỗi file, chunk nhị phân 8 KiB và tối đa 8 luồng song
  song. File lớn được staging trên đĩa, có checksum từng chunk, resume
